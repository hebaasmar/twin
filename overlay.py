"""
Twin — Real-time context engine.

Run:  python3 overlay.py
Then: browser opens automatically at http://localhost:5055

Features:
- Three-panel UI (Sessions / Stage / Agent)
- Start Meeting: Begins continuous transcription
- Spacebar: Hold to capture audio, release to get Claude answer
- Agent chat: Talk to Twin before/during/after the meeting
...
deepgram_ws = None
deepgram_ws_lock = threading.Lock()
            ...
            if fname.endswith(".json"):
                ...
                try:
                    with open(fpath) as f:
                        data = json.load(f)
                    has_content = len(data.get("exchanges", data.get("qa_pairs", []))) > 0
                    is_active = data.get("is_meeting_active", False)
                    # Orphaned = claims active but isn't our current session in memory
                    orphaned = is_active and (current_session.get("id") != data.get("id"))

                    # Auto-lock orphaned sessions (browser closed / server restarted)
                    # This makes the model simple: meeting ends when connection lost
                    if orphaned and has_content:
                        data["is_meeting_active"] = False
                        data["locked"] = True
                        with open(fpath, "w") as f:
                            json.dump(data, f, indent=2)
                        log_info(f"Auto-locked orphaned session {data.get('id')}")
...
# ── Continuous Transcription (Deepgram) ───────────────────────────────────────
def start_continuous_transcription():
    """Start continuous mic transcription via Deepgram WebSocket."""
    global continuous_active, continuous_thread, continuous_stop_event, deepgram_ws

    if not DEEPGRAM_API_KEY:
        log_error("DEEPGRAM_API_KEY not set in .env")
        return
        ...
        try:
            ...
            if data.get("type") == "Results":
                ...
                if alternatives:
                    ...
                    if transcript:
                        # Get speaker info from words if available
                        ...
                        log_info(f"[DEEPGRAM] {speaker or 'Speaker'}: {transcript[:50]}...")
                        ...
                        current_session["transcript"] = existing + transcript + " "
            ...
            log_error(f"Deepgram message error: {e}")

    def on_error(ws, error):
        log_error(f"Deepgram WebSocket error: {error}")

    def on_close(ws, close_status_code, close_msg):
        log_info(f"Deepgram WebSocket closed: {close_status_code} - {close_msg}")

    def on_open(ws):
        log_info("Deepgram WebSocket connected, starting audio stream...")
        ...
        def send_audio():
            try:
                ...
                log_info("Continuous transcription started (Deepgram)...")
                ...
                log_info("Continuous transcription stopped (Deepgram).")
    ...
    def run_websocket():
        global deepgram_ws
        try:
            # Deepgram streaming URL with diarization enabled
            url = (
            ...
            deepgram_ws.run_forever(sslopt={"cert_reqs": ssl.CERT_NONE})

        except Exception as e:
            log_error(f"Deepgram WebSocket error: {e}")
            traceback.print_exc()

    continuous_thread = threading.Thread(target=run_websocket, daemon=True)
    continuous_thread.start()


def stop_continuous_transcription():
    """Stop continuous transcription."""
    global continuous_active, continuous_stop_event, deepgram_ws
    continuous_stop_event.set()
    continuous_active = False

    # Close Deepgram WebSocket
    with deepgram_ws_lock:
        if deepgram_ws:
            try:
                deepgram_ws.close()
            except:
...
def stop_recording_and_transcribe():
    """Stop recording and transcribe via Deepgram pre-recorded API."""
    ...
    try:
        # Create WAV file from captured audio
        ...
        log_info(f"Transcribing {len(audio_frames)} frames via Deepgram...")

        # Use Deepgram pre-recorded API
        ...
        try:
            ...
            log_error(f"Deepgram API error: {e.code} - {e.read().decode()}")
            os.unlink(tmp_path)
            return None
...
# ── Meeting API routes (for new frontend) ─────────────────────────────────────
@app.route("/api/meeting/status")
def api_meeting_status():
    """Return current meeting status."""
    return jsonify({
        "active": meeting_active,
        "session_id": current_session.get("id") if meeting_active else None,
    })


@app.route("/api/meeting/start", methods=["POST"])
def api_meeting_start():
    """Start a meeting - alias to start_meeting_route."""
    return start_meeting_route()


@app.route("/api/meeting/end", methods=["POST"])
def api_meeting_end():
    """End a meeting - alias to stop_meeting_route."""
    return stop_meeting_route()


@app.route("/api/meeting/pause", methods=["POST"])
def api_meeting_pause():
    """Pause a meeting - stop transcription but keep session active."""
    global meeting_active
    if not meeting_active:
        return jsonify({"active": False})
    stop_continuous_transcription()
    log_info("Meeting paused - transcription stopped")
    return jsonify({"active": True, "paused": True, "session_id": current_session.get("id")})


@app.route("/api/meeting/resume", methods=["POST"])
def api_meeting_resume():
    """Resume a paused meeting."""
    global meeting_active
    if not meeting_active:
        return jsonify({"active": False})
    start_continuous_transcription()
    log_info("Meeting resumed - transcription started")
    return jsonify({"active": True, "paused": False, "session_id": current_session.get("id")})


@app.route("/api/capture/start", methods=["POST"])
def api_capture_start():
    """Start recording for Q&A capture."""
    return start_recording_route()


@app.route("/api/capture/stop", methods=["POST"])
def api_capture_stop():
    """Stop recording and process Q&A."""
...
def api_session_lock(session_id):
    """Lock a session permanently."""
    data = load_session(session_id)
    if not data:
        return jsonify({"error": "Session not found"}), 404
    data["locked"] = True
...
def stop_meeting_route():
    ...
    conversation_history = []
    current_session["is_meeting_active"] = False
    current_session["locked"] = True  # Lock the session permanently
    save_session()
...
def process_recording():
    ...
    try:
        ...
        try:
            ...
            system_prompt = "You are Twin, a real-time context engine. Generate spoken beats."
...
<div class="spaces">
  ...
  <div class="agent" id="agentPanel">
    ...
    <div class="agent-thread" id="agentThread">
      <div class="placeholder-text">Talk to Twin before, during, or after the meeting...</div>
    </div>
...
function renderAgentThread(sessionData) {
  ...
  if (!messages.length) {
    thread.innerHTML = '<div class="placeholder-text">Talk to Twin before, during, or after the meeting...</div>';
    return;
  }
...
if __name__ == "__main__":
    if not DEEPGRAM_API_KEY:
        log_error("DEEPGRAM_API_KEY not set in .env file")
        sys.exit(1)
    log_info("Deepgram API key loaded.")
